...
function OverlayInfo({ title, averageScore, genres }) {
    ...
    return (
        <div className="anime-details-overlay absolute bottom-0 left-0 w-full p-4 text-white backdrop-blur-md rounded-b-xl  ">
            ...
            <div className='flex font-normal text-xs'>
                {genres.slice(0, 3).map((genre, idx) => (
                    <React.Fragment key={idx}>
                        <p>{genre}</p>
                        {idx !== 2 && idx !== genres.slice(0, 3).length - 1 && <span>,&nbsp;</span>}
                    </React.Fragment>
                ))}
            </div>
        </div>
